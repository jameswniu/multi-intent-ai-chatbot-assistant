...
```mermaid
%%{init: {'theme': 'base', 'flowchart': {'curve': 'linear', 'nodeSpacing': 44, 'rankSpacing': 44, 'padding': 18}, 'themeVariables': {'primaryColor': '#1a1f26', 'primaryTextColor': '#e6e8eb', 'primaryBorderColor': '#2b3138', 'lineColor': '#6b7f99', 'fontSize': '16px'}}}%%
flowchart TD
    G[guard] --> R1[refused, a known phrase]
    G --> C[classify]
    C --> S[contract, select columns]
    C --> F[knowledge, retrieve]
    C --> A[tie, ask again]
    S --> V[validate]
    V --> D[(contracts)]
    V --> R2[refused, wrong shape]
    D --> P[redact]
    F --> P
    P --> O[answer]
